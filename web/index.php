...
//删除10天前的每个台站最后状态数据包
	$q="delete from aircraftlast where tm<=date_sub(now(),INTERVAL 10 day)";
	$mysqli->query($q);
	...
	if (isset($_REQUEST["aid"]))  {
		$aid=$_REQUEST["aid"];
		$q="select lat,lon,aid,unix_timestamp(tm),tm,icao,alt,speed,h,vr from aircraftlast where (aid=? or (tm>=FROM_UNIXTIME(?) and tm>=?) )";
		$stmt=$mysqli->prepare($q);
        	$stmt->bind_param("sis",$aid,$tm,$startdatestr);
	} else {
	...
	$q="select lat,lon from aircraftlog where tm>? and `aid`=? order by tm limit 50000 offset ?";
        $stmt=$mysqli->prepare($q);
        $stmt->bind_param("ssi",$startdatestr,$aid,$pathlen);
        $stmt->execute();
...
function disp_map($aid) {
	echo "<a href=\"".$_SERVER["PHP_SELF"]."?map&aid=".$aid."\" target=_blank>本站</a> ";
}
...
function monitor_station(myaid) {
	...
	aid=myaid;
	if(ismobile) 
		document.getElementById("msg").innerHTML = aid;
	else
		document.getElementById("msg").innerHTML = " 跟踪"+aid;
	if(autocenter)
...
//	alert(lastupdatetm);
	var b = map.getBounds();
        var url = window.location.protocol+"//"+window.location.host+":"+window.location.port+"/"+window.location.pathname+"?tm="+lastupdatetm+"&aid="+aid+"&pathlen="+movepath.length+"&llon1="+llon1+"&llon2="+llon2+"&llat1="+llat1+"&llat2="+llat2+"&lon1="+b.getSouthWest().lng+"&lat1="+b.getSouthWest().lat+"&lon2="+b.getNorthEast().lng+"&lat2="+b.getNorthEast().lat+"&disp15min="+disp15min+"&ldisp15min="+ldisp15min;
	url = url+"&lspan="+lspan;
...
*/
	...
	header("Content-Disposition:attachment; filename=\"".$aid."-".date("Y-m-d")."Track.gpx\"");
	echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
	echo "<gpx version=\"1.0\">\n";
	echo "<name>APRS GPX</name>\n";
	echo "<trk><name>".$aid." ".date("Y-m-d")." Track</name><number>1</number><trkseg>\n";
	$q="select date_format(CONVERT_TZ(tm,@@session.time_zone, '+00:00'),\"%Y-%m-%dT%H:%i:%sZ\"),lat,lon,msg,datatype from aprspacket where tm>? and `aid`=? and lat<>'' and not lat like '0000.00%' order by tm";
        $stmt=$mysqli->prepare($q);
        $stmt->bind_param("ss",$startdatestr,$aid);
        $stmt->execute();
...
if($cmd=="kml") {
	...
	date_default_timezone_set("Asia/Shanghai");
	header("Content-Type:application/gpx+xml");
	header("Content-Disposition:attachment; filename=\"".$aid."-".date("Y-m-d")."Track.kml\"");
	echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
	echo "<kml xmlns=\"http://www.opengis.net/kml/2.2\" xmlns:gx=\"http://www.google.com/kml/ext/2.2\">\n";
	echo "<Document>\n";
	echo "<name>".$aid." Track</name>\n";?>
	<Style id="ylw">
		<LabelStyle>
...
<?php
	echo "<Folder><name>".$aid."-".date("Y-m-d")."</name>\n";
	echo "<Placemark>\n";
...
if ($cmd=="new") {
	echo "<h3>最新收到的ADSB数据包</h3>";
	$q="select tm,icao,aid,raw from aircraftlog where tm>=curdate() order by tm desc limit 10";
	$result = $mysqli->query($q);
	echo "<table border=1 cellspacing=0><tr><th>时间</th><th>ICAO</th><th>AID</th><th>地图</th></tr>\n";
	while($r=$result->fetch_array()) {
        	echo "<tr><td>";
        	echo $r[0];
        	echo "</td><td>";
        	echo "<a href=".$_SERVER["PHP_SELF"]."?aid=$r[1]>$r[2]</a>";
        	echo "</td><td>";
		echo $r[2];  //raw
        	echo "</td><td>";
		disp_map($r[1]);
